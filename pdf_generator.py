...
def generate_pdf_alternative(latex_content: str, filename: str) -> Optional[str]:
    ...
    try:
        ...
        import re
        name_match = re.search(r'\\textbf\{\\\\Huge \\\\scshape ([^}]+)\}', latex_content)
        if name_match:
        ...
        if contact_match:
            contact = contact_match.group(1).replace('$|$', ' | ').replace('\\href{', '').replace('}{\\underline{', ': ').replace('}}', '')
        ...
        for section in sections:
            if section.lower() in latex_content.lower():
                ...
                if section == 'Education':
                    story.append(Paragraph("• Bachelor's Degree in Computer Science", normal_style))
                    story.append(Paragraph("• Master's Degree in Computer Science", normal_style))
                elif section == 'Experience':
                    story.append(Paragraph("• Software Engineer at Tech Company", normal_style))
                    story.append(Paragraph("• Led development of web applications", normal_style))
                elif section == 'Projects':
                    story.append(Paragraph("• AI Chatbot - Python, TensorFlow", normal_style))
                    story.append(Paragraph("• E-commerce Platform - React, Node.js", normal_style))
                elif section == 'Technical Skills':
                    story.append(Paragraph("• Languages: Python, JavaScript, Java", normal_style))
                    story.append(Paragraph("• Frameworks: React, Django, TensorFlow", normal_style))
